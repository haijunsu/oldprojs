...
<TABLE border="0" width="100%"> 
	...
	<TR bgcolor="#55CCCC" align="center" Height="25"><TD>课程名称</TD><TD>课程类型</TD><TD>学习进度</TD><TD>开始学习</TD><TD>参加测试</TD></TR>
<%
...
%>
	<TR>
		<TD><A href=<%=request.getContextPath()%>/lesson/lessonInfo.jsp?classid=<%=lessonScheduleBgd.getFieldValue("class_id", i)%> target=_blank><%=lessonScheduleBgd.getFieldValue("class_name", i)%></A>
		</TD>
		<TD>
		...
		<TD>
			<%if(Integer.parseInt(lessonScheduleBgd.getFieldValue("pass_stander", i))>0){%><A href=<%=request.getContextPath()%>/exam/lessonExam.jsp?classid=<%=lessonScheduleBgd.getFieldValue("class_id", i)%> target=test>测验</A>	
			<%}else{out.print("&nbsp;");}%>
		</TD>
	</TR>
<% } %>
