...
String lessonScheduleSql = "SELECT uc.user_id AS user_id, uc.class_id AS class_id, c.class_name AS class_name,c.pass_stander as pass_stander FROM t_user_class uc, t_user u, t_class c WHERE uc.class_id = c.class_id AND uc.status = '1' AND (uc.user_id = u.user_id OR uc.user_id = u.group_id) AND c.pass_stander >0 AND u.user_id = '" + lessonScheduleUserid + "'";
lessonScheduleBgd.executeSelect(lessonScheduleSql);
%>
<TABLE border="0" width="100%"> 
	<TR>
		<TD><B>我的考试表</B>================(共<%=lessonScheduleBgd.getRowCount()%>门课程需要考试)</TD>
	</TR>
	<TR>
	<TD>
	<!-- 开始显示课程表 -->
	<TABLE width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#eeeeee">
	<TR bgcolor="#55CCCC" align="center" Height="25"><TD>课程名称</TD><TD>课程类型</TD><TD>学习进度</TD><TD>测验</TD></TR>
<%
...
%>
	<TR>
		...
		<TD>
			<A href=../exam/lessonExam.jsp?classid=<%=lessonScheduleBgd.getFieldValue("class_id", i)%> target=test>&nbsp;测验</A>			
		</TD>
