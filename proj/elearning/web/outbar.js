...
while(i>0)
{
	...
	if(i==1)
	{
		document.write("<input class='input' position='UP' border=0 id='OB_Button1' onDblClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' onClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' TYPE='button' value='"+Folder[0]+"' style='position:absolute;top:0;left:0;width:151;height:"+OB_ButtonHeight+";font-family:"+OB_ButtonFontFamily+";font-size:"+OB_ButtonFontSize+"pt;cursor:hand;color:"+OB_ButtonFontColor+";z-index:100'>");
		MakeItems(Folder,i,OB_ButtonHeight);	
	}	
	else
	{
		document.write("<input  class='input' position='DOWN' id='OB_Button"+i+"' border=0 onDblClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' onClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' TYPE='button' value='"+Folder[0]+"' style='position:absolute;top:"+(OB_Height-(j-i)*OB_ButtonHeight-OB_BorderWidth*2)+";left:0;width:151;height:"+OB_ButtonHeight+";font-family:"+OB_ButtonFontFamily+";font-size:"+OB_ButtonFontSize+"pt;cursor:hand;color:"+OB_ButtonFontColor+";z-index:100'>");
...
function MakeItems(Folder,zorder,top)
{
	...
	document.write("<DIV id='OB_Folder"+zorder+"' style='position:absolute;left:45;top:"+top+";width:130;height:"+(OB_Margin*2+items*(OB_IconsHeight+OB_LabelFontSize+OB_LabelMargin)+(items-1)*OB_ItemsSpacing)+";z-index:"+zorder+";clip:rect(0 0 0 0);'>");
	for(var i=1;i<items*4;i+=4)
	{
		//document.write("<div targetFrame='"+Folder[i+3]+"' link='"+Folder[i+2]+"' onMouseDown='OutlookLikeBar.ItemClicked(this)' onMouseUp='OutlookLikeBar.ItemSelected(this)' onMouseOver='OutlookLikeBar.OverItems(this)' onMouseOut='OutlookLikeBar.OutItems(this)' style='position:absolute;left:"+(Math.ceil((OB_Width-OB_BorderWidth*2-OB_IconsHeight)/2)-1)+";top:"+(OB_Margin+Math.ceil((i-1)/4)*(OB_ItemsSpacing+OB_LabelFontSize+OB_IconsHeight))+";cursor:hand;clip:rect(0 "+OB_IconsWidth+" "+OB_IconsHeight+" 0;width:"+OB_IconsWidth+";height:"+OB_IconsHeight+"'>");
 		...
 		//document.write("</div>");
		if (Folder[i+3] == "_blank")
			document.write("<div targetFrame='"+Folder[i+3]+"' link='"+Folder[i+2]+"' onMouseDown='OutlookLikeBar.ItemClicked(this)' onMouseUp='OutlookLikeBar.ItemSelected(this)' onMouseOver='OutlookLikeBar.OverItems(this)' onMouseOut='OutlookLikeBar.OutItems(this)' style='position:absolute;left:5;top:"+(OB_Margin+Math.ceil((i-1)/4)*(OB_ItemsSpacing+OB_LabelFontSize+OB_IconsHeight))+";cursor:hand; padding-top:4px;clip:rect(0 130 "+OB_IconsHeight+" 0;width:130;height:22;color:#133B4D'>");
		else
			document.write("<div targetFrame='parent."+Folder[i+3]+"' link='"+Folder[i+2]+"' onMouseDown='OutlookLikeBar.ItemClicked(this)' onMouseUp='OutlookLikeBar.ItemSelected(this)' onMouseOver='OutlookLikeBar.OverItems(this)' onMouseOut='OutlookLikeBar.OutItems(this)' style='position:absolute;left:5;top:"+(OB_Margin+Math.ceil((i-1)/4)*(OB_ItemsSpacing+OB_LabelFontSize+OB_IconsHeight))+";cursor:hand; padding-top:4px;clip:rect(0 130 "+OB_IconsHeight+" 0;width:130;height:22;color:#133B4D'>");	
 		document.write("&nbsp;");
 		document.write("<a href='"+Folder[i+2]+"' target='"+Folder[i+3]+"' style='text-decoration:none'>" );
 		document.write("<img src='"+Folder[i]+"' align='top' border=0>");
 		//document.write("&nbsp;&nbsp;");
 		//document.write("<span style='cursor:hand;color:#ffffff;width:80'>");
		...
		//document.write("<div align='center' style='position:absolute;left:40;top:"+(OB_LabelMargin+OB_IconsHeight+OB_Margin+Math.ceil((i-1)/4)*(OB_ItemsSpacing+OB_LabelFontSize+OB_IconsHeight))+";font-family:"+OB_LabelFontFamily+";font-size:8pt;color:"+OB_LabelFontColor+"'>");		
		document.write(Folder[i+1]);
		document.write("</div>");
	}	
	document.write("</DIV>");
...
function OverItems(item)
{
	if(this.sliding)
		return;		
	//item.style="border-top:1 solid #ffffff;border-left:1 solid #ffffff;border-right:1 solid #287dA3;border-bottom:1 solid #287dA3;";
	item.className="bor";
}
