...
s_sql="select class_id, class_name, sum(peoples) count from t_class group by class_id, class_name order by count desc";

/*
s_total="select "+strSubstr+"("+"class_id"+",1,8) id"+",sum(peoples) count from t_class group by "+strSubstr+"("+"class_id"+",1,8)"+" order by count desc";
*/
s_total="select a.class_id as id,count(a.class_id) as count from t_user_class a,t_user  b where a.user_id=b.user_id  group by a.class_id union select  a.class_id as id,count(a.class_id) as count from t_user b,t_user_class a where b.group_id=a.user_id group by a.class_id";
...
<head>
<link REL="stylesheet" TYPE="text/css" HREF="<%=request.getContextPath()%>/include/css/hongdicrm.css">
<style>
...
<TR><TD>课程名称(访问次数前10名)</TD><TD><INPUT TYPE="checkbox" NAME="col1">正在学习人数</TD><TD><INPUT TYPE="checkbox" NAME="col2">访问次数</TD></TR>
<%
...
{
 class_id=(bgd.getFieldValue("class_id",m));
