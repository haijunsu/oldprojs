...
<table cellpadding=0 cellspacing=0 border=0 width=770 align=center>
  <tr>
    <td valign=bottom><font color=#000077><img src=<%=request.getContextPath()%>/images/closedfold.gif>
      ...
      <img src=<%=request.getContextPath()%>/images/openfold.gif width="12" height="12">&nbsp<%=request.getAttribute("title")%></td>
...
<%if(request.getAttribute("action").equals("update")) {%>
	...
	<input type=text maxlength="20" name="userid" value="<%=beanUserInfo.getUserid()%>" readonly>&nbsp;<font color=red>* 此项不能更改</td>
<%}else{%>
	<td bgcolor=#d7d7d7 width=60%>
	<input type=text maxlength="20" name="userid" value="<%=beanUserInfo.getUserid()%>">&nbsp;<input onClick="javascript:Check()" type=button value="检测帐号" name="button" class="button">&nbsp;* 此项必须填写</td>
<%}%>
<td width=17 background=<%=request.getContextPath()%>/images/kright.gif bgcolor=#d7d7d7></td>
	<tr>
        ...
        <td bgcolor=#e7e7e7 width=60%><input type=password name="pass" maxlength=20 value="<%=beanUserInfo.getPassword()%>">&nbsp;* 此项必须填写</td><td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
        </tr>
        	<tr><td width=17 background=<%=request.getContextPath()%>/images/kpartleft.gif bgcolor=#e7e7e7></td>
        <td bgcolor=#e7e7e7><input type=password name="repass" maxlength=20 value="<%=beanUserInfo.getPassword()%>">&nbsp;* 再输一遍，以便确定！</td><td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
        </tr>
		<tr>
...
<td bgcolor=#d7d7d7><input type=text name="email" value="<%=beanUserInfo.getEmail()%>">&nbsp;* 此项必须填写</td><td width=17 background=<%=request.getContextPath()%>/images/kright.gif bgcolor=#d7d7d7></td></tr>
		<tr>
        ...
        <tr>
			...
			<td bgcolor=#e7e7e7><input type=text name="company" size=50 maxlength=50 value="<%=beanUserInfo.getCompany()%>"></td>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
		</tr>
		<tr>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartleft.gif bgcolor=#e7e7e7></td>
			<td bgcolor=#e7e7e7 height=40><font color=#000077><b>所在部门：</b>配合上栏使用<br>最大 20 个字节（10个汉字）</td> 
			<td bgcolor=#e7e7e7><input type=text name="department" value="<%=beanUserInfo.getDepartment()%>" size=20 maxlength=20 value="<%=beanUserInfo.getDepartment()%>"></td><td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
		</tr>
		<tr>
          <td width=17><img src=<%=request.getContextPath()%>/images/ksplitleft.gif width=17 height="2" border=0></td>
          <td background=<%=request.getContextPath()%>/images/ksplitmiddle.gif height=2 width=100% colspan=2><img src=<%=request.getContextPath()%>/images/none.gif height=2 border=0></td>
...
<%if(session.getAttribute("userid")!=null) {if(ets.isAdmin(session.getAttribute("userid").toString())){ %>
        <tr>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartleft.gif bgcolor=#e7e7e7></td>
			<td bgcolor=#e7e7e7 height=40><font color=#000077><b>用户组：</b>建议您填写<br>最大 50 个字节（25个汉字）</td> 
			<td bgcolor=#e7e7e7>
			<select id='group_id' name='group_id' size='1'>
				...
				{
				%>
				<option <%=bqc.getCodeValue("code_value",j).trim().equals(beanUserInfo.getGroupid().trim())?"SELECTED":""%> value='<%=bqc.getCodeValue("code_value", j)%>'><%=bqc.getCodeValue("code_namec", j)%></option>
				<%}%>
			</select>
			</td>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
		...
		<tr>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartleft.gif bgcolor=#e7e7e7></td>
			<td bgcolor=#e7e7e7 height=40><font color=#000077><b>权限：</b>配合上栏使用<br>最大 20 个字节（10个汉字）</td> 
			<td bgcolor=#e7e7e7>
			<%
			boolean isChecked;
			bqc.QueryCode("0003");
			for (int j=0; j<bqc.getCodeCount(); j++)
			{
				isChecked = ets.isMenuShow(ets.parseRight(bqc.getCodeValue("code_value", j)), beanUserInfo.getUserRight());
			%>
		...
		<tr>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartleft.gif bgcolor=#e7e7e7></td>
			<td bgcolor=#e7e7e7 height=40><font color=#000077><b>用户状态：</b>配合上栏使用<br>最大 20 个字节（10个汉字）</td> 
			<td bgcolor=#e7e7e7>
			<select id='user_status' name='user_status' size='1'>
				...
				{
				%>
					<option <%=bqc.getCodeValue("code_value", j).equals(beanUserInfo.getUserStatus())?"SELECTED":""%> value='<%=bqc.getCodeValue("code_value", j)%>'><%=bqc.getCodeValue("code_namec", j)%></option>
				<%}%>
			</select>
			</td>
			<td width=17 background=<%=request.getContextPath()%>/images/kpartright.gif bgcolor=#e7e7e7></td>
...
<td bgcolor=#d7d7d7>
<SELECT name="userxz" onchange=showxz() size="1">
<OPTION value="">保密</OPTION>
...
<td width=50%><INPUT id=advcheck name=advshow type=checkbox value=1 onclick=showadv()><span id="advance">显示高级注册选项</a></span> </td>
<td width=50%>
<input type=submit value="<%=request.getAttribute("action").equals("update")?"修 改":"注 册"%>" onclick="return valid()"></td>
