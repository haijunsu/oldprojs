...
public class beanCodeManager extends HttpServlet implements java.io.Serializable
{
	...
	{
		String strLoginPage, strPage, strOkPage, strErrPage;
		try
		{
			...
			s_reload = getParameter(req, "reload", true, false, false, "true");
			s_treeRoot = getParameter(req, "root", true, false, false, "0000");
			req.setAttribute("treeroot", s_treeRoot);
			req.setAttribute("reload", s_reload);
		...
		catch(Exception ex)
		{
			...
			{
				strErrPage = getParameter(req, "errPage", false, true, true, "");
				System.err.println("beanCodeManager error: " + ex);
			...
			{
				System.err.println("beanCodeManager调用错误页面时出错！");
				e.printStackTrace();
			}
		}
    }
