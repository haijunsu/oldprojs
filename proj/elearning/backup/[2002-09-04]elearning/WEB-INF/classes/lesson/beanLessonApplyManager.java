...
public class beanLessonApplyManager extends HttpServlet implements java.io.Serializable
{
	...
	public void doProcess(HttpServletRequest req, HttpServletResponse res) throws ServletException
	{
		...
		try
		{
			...
			s_page = getParameter(req, "page", false, true, false, "true");
...
//			session.setAttribute("applyType", s_applyType);
		...
		catch(Exception ex)
		{
			...
			{
				strErrPage = getParameter(req, "errPage", false, true, true, "");
				System.err.println("beanLessonApplyManager error: " + ex);
			...
			{
				System.err.println("beanLessonApplyManager调用错误页面时出错！");
				e.printStackTrace();
			}
		}
    }
