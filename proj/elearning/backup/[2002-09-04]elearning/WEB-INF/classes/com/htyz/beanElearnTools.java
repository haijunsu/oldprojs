...
public class beanElearnTools
{
	...
	public int check_date(String date_str)
	{
		...
		if (int_day==31&&(int_month==4||int_month==6||int_month==9||int_month==11))
		{
			return -8;
		}
		//判断是否为闰年来确定2月份的最大天数
	...
	public String AutoNum(String strNum)
	{
		...
		if(numLen==0)
		{
			...
			rtnNum = rtnNum.substring(rtnNum.length() - numLen, rtnNum.length());
			return rtnNum; 
		}
	}
	...
	{
		boolean processURLChinese = com.htyz.system.SystemProperties.getBooleanProperty("processURLChinese");
    ...
    public boolean isAdmin(String userid)
    {
    	...
    	bgd.executeSelect("select user_right from t_user where user_id = '" + userid + "'");
    	if (bgd.getRowCount() == 0)
    		return false;
    	return isMenuShow(parseRight("6"), bgd.getFieldValue("user_right", 0));
    }
	...
	{
		String s_navigation = "<A href=\"/elearning/\" target=\"_top\">首页</A>"
						+ "&gt;<A href=\"lessonManager\" target=\"_top\">课程管理</A>";;
		return s_navigation;
	}
	
	public String getClassNavigation(String classid)
	{
		...
		s_navigation = getClassNavigation() + "&gt;<A href=\"lessonList?action=showDetail&codeid=0013&codevalue=" + temp + "\">" + bqc.getCodeValue("code_namec", 0) + "</A>";
		
		if (classid.length() >= 8 )
		{
			bqc.QueryCode(temp, classid.substring(4, 8));
			s_navigation = s_navigation + "&gt;<A href=\"lessonList?action=showDetail&codeid=" + temp + "&codevalue=" + classid.substring(4, 8) + "\">" + bqc.getCodeValue("code_namec", 0) + "</A>";
		}
		return s_navigation; 
	}
	
	public String getClassNavigation(String classid, String lessonid)
	{
		String s_navigation;
		beanGetdata bgd = new beanGetdata();
		s_navigation = getClassNavigation(classid);
		
		bgd.executeSelect("SELECT class_name FROM t_class WHERE class_id = '" + classid + "'");
		s_navigation = s_navigation +  "&gt;<A href=\"chapterList?reload=true&classid=" + classid + "\" target=left>" + bgd.getFieldValue("class_name", 0) + "</A>";
		
		if (!lessonid.endsWith("000"))
		{
			lessonid = lessonid.substring(0, 3) + "000";
			bgd.executeSelect("SELECT lesson_name FROM t_lesson WHERE class_id = '" + classid + "' AND lesson_id = '" + lessonid + "'");
			s_navigation = s_navigation +  "&gt;<A href=\"chapterList?action=showDetail&classid=" + classid + "&lessonid=" + lessonid + "\">" + bgd.getFieldValue("lesson_name", 0) + "</A>";
		}
		return s_navigation;
	}
...
//		return str;
	...
	public String strReplace(String fString,String olds,String news)
	{
		...
		while(Startwith!=-1){
			fString=fString.substring(0,Startwith)+news+fString.substring(Startwith+olds.length());
	...
	public String FormatFlash(String fString)
	{
		...
		while(Startwith!=-1)
		{
			...
			fString=fString.substring(0,Startwith)+tmpInsert+fString.substring(Stopwith+8);
			Startwith=fString.indexOf("[flash]");
	...
	public String FormatImage(String fString)
	{
		...
		while(Startwith!=-1)
		{
			...
			fString=fString.substring(0,Startwith)+tmpInsert+fString.substring(Stopwith+6);
			Startwith=fString.indexOf("[img]");
...
//将网页中的UBB字串换成标准HTML格式
	...
	public String strFormat(String fString)
	{
		...
		fString = strReplace(fString, "[font=Andale Mono]", "<font face='Andale Mono'>");
		fString = strReplace(fString, "[/font=Andale Mono]", "</font id='Andale Mono'>");
		fString = strReplace(fString, "[font=Arial]", "<font face='Arial'>");
		fString = strReplace(fString, "[/font=Arial]", "</font id='Arial'>");
		fString = strReplace(fString, "[font=Arial Black]", "<font face='Arial Black'>");
		fString = strReplace(fString, "[/font=Arial Black]", "</font id='Arial Black'>");
		fString = strReplace(fString, "[font=Book Antiqua]", "<font face='Book Antiqua'>");
		fString = strReplace(fString, "[/font=Book Antiqua]", "</font id='Book Antiqua'>");
		fString = strReplace(fString, "[font=Century Gothic]", "<font face='Century Gothic'>");
		fString = strReplace(fString, "[/font=Century Gothic]", "</font id='Century Gothic'>");
		fString = strReplace(fString, "[font=Comic Sans MS]", "<font face='Comic Sans MS'>");
		fString = strReplace(fString, "[/font=Comic Sans MS]", "</font id='Comic Sans MS'>");
		fString = strReplace(fString, "[font=Courier New]", "<font face='Courier New'>");
		fString = strReplace(fString, "[/font=Courier New]", "</font id='Courier New'>");
		...
		fString = strReplace(fString, "[font=Times New Roman]", "<font face='Times New Roman'>");
		fString = strReplace(fString, "[/font=Times New Roman]", "</font id='Times New Roman'>");
		fString = strReplace(fString, "[font=Trebuchet MS]", "<font face='Trebuchet MS'>");
		fString = strReplace(fString, "[/font=Trebuchet MS]", "</font id='Trebuchet MS'>");
		fString = strReplace(fString, "[font=Script MT Bold]", "<font face='Script MT Bold'>");
		fString = strReplace(fString, "[/font=Script MT Bold]", "</font id='Script MT Bold'>");
		fString = strReplace(fString, "[font=Stencil]", "<font face='Stencil'>");
		fString = strReplace(fString, "[/font=Stencil]", "</font id='Stencil'>");
		fString = strReplace(fString, "[font=Lucida Console]", "<font face='Lucida Console'>");
		fString = strReplace(fString, "[/font=Lucida Console]", "</font id='Lucida Console'>");	
		...
		fString = strReplace(fString,"[/*]", "</li>");
		fString = strReplace(fString,"[quote]", "<BLOCKQUOTE id=quote><font id=quote><hr height=1 noshade id=quote>");
		fString = strReplace(fString,"[/quote]", "<hr height=1 noshade id=quote></font id=quote></BLOCKQUOTE id=quote>");
		fString = strReplace(fString,"[fly]", "<marquee width=90% behavior=alternate scrollamount=3>");
		...
		fString = strReplace(fString,"[quote]", "<BR><table cellpadding=0 cellspacing=0 border=0 WIDTH=94% bgcolor=#000000 align=center><tr><td><table width=100% cellpadding=5 cellspacing=1 border=0><TR><TD BGCOLOR=#F8F8F8><font id=quote>");
		fString = strReplace(fString,"[/quote]", "</td></tr></table></td></tr></table>");
		fString = strReplace(fString,"[quote]", "<table cellpadding=0 cellspacing=0 border=0 WIDTH=94% bgcolor=#000000 align=center><tr><td><table width=100% cellpadding=5 cellspacing=1 border=0><TR><TD BGCOLOR=#F8F8F8><font id=quote>");
	...
	public String getSignature(String userid){
		beanGetdata bgd = new beanGetdata();
    	bgd.executeSelect("select signature from t_user where user_id = '" + userid + "'");
    	if(bgd.getRowCount()<1){
    		return "";
    	}else{
    		return "<BR><BR>------------------------------------------------------------------------------------<BR>"+strFormat(bgd.getFieldValue(1,0));
    	}
    }
	public String getDBDate() {
		beanGetdata getData = new beanGetdata();
		getData.executeSelect("SELECT getdate() AS getdate FROM t_code WHERE code_id = '0000' AND code_value = '0001'");
