...
%>
      	<A href=/elearning/lesson/lessonInfo.jsp?showback=show&classid=<%=bgd.getFieldValue("class_id", i+(pageNum-1)*listCount)%> title=<%=title + "……"%>><%=bgd.getFieldValue("class_name", i+(pageNum-1)*listCount)%></A>&nbsp;
<%		//}
		//else
...
<%		if (!applyType.equals("free"))
		{
			...
			String s_reason = "";
			String temp = bgd.getFieldValue("class_id", i+(pageNum-1)*listCount) + "," + bgd.getFieldValue("class_name", i+(pageNum-1)*listCount);
