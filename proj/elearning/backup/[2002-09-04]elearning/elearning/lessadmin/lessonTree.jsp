...
String oldopen = (String)session.getAttribute("lessonTree.oldopen");
...
<SCRIPT language="JavaScript">
	parent.right.location.href = "lessonList?action=<%=action%>&codeid=<%=codeid%>&codevalue=<%=codevalue%>&message=<%=message%>";
</SCRIPT>
<%
}
%>
...
<jsp:useBean id="tree" scope="request" class="com.htyz.treeBeans.CSTreeBean" />
  ...
  <jsp:setProperty name="tree" property="treePage" value="lessonList" />
...
<TABLE>
  <TBODY>
    <TR>
      <TD><%= tree.renderHTML() %>
      <%
      	session.setAttribute("lessonTree.oldopen", tree.getOldopen());
      %>
      </TD>
    </TR>
  </TBODY>
