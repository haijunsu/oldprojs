...
public class beanLessonNOte extends HttpServlet implements java.io.Serializable
{
		...
		try
		{
			...
			session.setAttribute("applyType", s_applyType);
				...
				s_navigation = "<A href=\"/elearning/\" target=\"_top\">首页</A>"
							+ "&gt;<A href=\"lessonApplyManager?applyType=" + s_applyType + "\" target=\"_top\">";
...
//					if (session.getAttribute(s_applyType + ".reason")!=null)
					...
					s_sql = "SELECT * FROM t_class WHERE substring(class_id, 1, 8) = '" + s_codeId + s_codeValue + "'  AND class_status = '1' " + s_whereType
						...
						else
						{
							...
							for (int i=0; i<v_selectedItems.size(); i++)
							{
								...
								//给申请表填记录
									...
									{
										s_sql = "INSERT INTO t_user_apply (user_id, class_id, apply_time, apply_reason, apply_status) "
												+ "VALUES ('" + s_userId + "', "
												+ "'" + s_classId + "', "
												+ "getdate(), "
												+ "'" + s_reason + "', "
