...
//添加新闻
			if (s_Action.equals("add"))
			{
			    ...
			    ss_sql = "SELECT max(news_id) as maxid FROM t_news";
			        ...
			        //newstype=req.getParameter("news_type");
				    newstitle = getParameter(req, "news_title", true, false, true, "");
				    //newstitle=req.getParameter("news_title");
				    newskey= getParameter(req, "news_key", true, false, true, "");
				   // newskey=req.getParameter("news_key");
				   newscontent=getParameter(req, "news_content", true, false, true, "");
				   //newscontent=req.getParameter("news_content");
				    newsman=getParameter(req, "news_man", true, false, true, "");
				    //newsman=req.getParameter("news_man");
		           newsimg=getParameter(req, "file1", true, false, false, "");
		           newshead=getParameter(req, "head", true, false, true, "");
				s_Sql = "INSERT INTO t_news (news_id, news_type,news_title,news_key,news_content,news_man,news_satuts,news_img,news_head) values('"+s_Newsid+"','"+newstype+"','"+newstitle+"','"+newskey+"','"+newscontent+"','"+newsman+"','1', '"+newsimg+"','"+newshead+"')";
			   // System.out.println(s_Sql);
				 bgd.execute(s_Sql);
		         getServletConfig().getServletContext().getRequestDispatcher("/news/newsadd.jsp").forward(req, res);	    
	             return;
	...
	try{
		...
		ss_sql="update t_news set news_title='"+title+"',news_type='"+type+"',news_key='"+key+"',news_content='"+content+"',news_man='"+man+"',news_img='"+img+"',news_head='"+head+"' where news_id='"+req.getParameter("newsid")+"' ";
		bgd.execute(ss_sql);
        getServletConfig().getServletContext().getRequestDispatcher("/news/newslist.jsp").forward(req, res);	    
	   ...
	   if(s_Action.equals("del")){
	   	req.setAttribute("action",s_Action);
	   	//String news_id=getParameter(req, "checkbox", true, false, true, "");
	   	//int news_count = Integer.parseInt(news_id);
	   	...
	   	String s_MsgId = "";
	   	//bgd.executeSelect("select * from t_news");
	   	...
	   	ss_sql="update t_news set news_satuts='0' where news_id in ("+s_MsgId+ ")";
