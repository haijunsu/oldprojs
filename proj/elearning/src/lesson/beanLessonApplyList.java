...
public class beanLessonApplyList extends HttpServlet implements
    ...
    public java.lang.String getParameter(
            ...
            java.lang.String parameterName, boolean checkRequestParameters,
            boolean checkInitParameters, boolean isParameterRequired,
    ...
    public void doProcess(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            String s_userId, s_pageNo, s_reload, s_open, s_action, s_codeId, s_codeValue, s_classId, s_className, s_classType, s_keywords, s_description, s_classTime, s_passStander, s_classStatus, s_message, s_navigation, s_queryId, s_queryColoum, s_catalog, s_page, s_typePage, s_listPage, s_countInPage, s_selectedItem, s_removeItems, s_applyType, s_sql;
            ...
            s_navigation = "<A href=\"" + req.getContextPath()
                    + "/\" target=\"_top\">首页</A>"
                    + "&gt;<A href=\"lessonApplyManager?applyType="
                    + s_applyType + "\" target=\"_parent\">";
            ...
            if (s_action.equalsIgnoreCase("showDetail")) {
                ...
                if (s_codeId.equals("0013")) {
                    s_sql = "SELECT * FROM t_code WHERE code_id = '"
                            + s_codeValue + "'";
                    s_display = getParameter(req, "typePage", false, true,
                            true, "/lesson/lessonApplyTypeList.jsp");
                    ...
                    s_display = getParameter(req, "listPage", false, true,
                            true, "/lesson/lessonApplyList.jsp");
                    bqc.QueryCode(s_codeId, s_codeValue);
                    s_catalog = bqc.getCodeValue("code_namec", 0);
                    s_navigation = s_navigation + "&gt;" + s_catalog;

                    //					if (session.getAttribute(s_applyType +
                    // ".selectedItems")!=null)
                    v_selectedItems = (Vector) session.getAttribute(s_applyType
                            + ".selectedItems");
                    //					if (session.getAttribute(s_applyType +
                    // ".oldSelectedItems")!=null)
                    v_oldSelectedItems = (Vector) session
                            .getAttribute(s_applyType + ".oldSelectedItems");
                    //					if (session.getAttribute(s_applyType + ".reason")!=null)
                    v_reason = (Vector) session.getAttribute(s_applyType
                            + ".reason");

                    v_approve = (Vector) session.getAttribute(s_applyType
                            + ".approve");
                    ...
                    if (v_selectedItems == null) //初始化数组，存贮用户已经选择的课程
                    {
                        ...
                        s_sql = "SELECT uc.class_id AS class_id, t.class_name AS class_name FROM t_user_class uc, t_class t WHERE uc.user_id = '"
                                + s_userId
                                + "' AND t.class_id = uc.class_id AND uc.status <> '9'"
                                + s_whereType;
                        bgd.executeSelect(s_sql);
                        //System.out.println(s_sql);
                        for (int i = 0; i < bgd.getRowCount(); i++) {
                            v_selectedItems.addElement(bgd.getFieldValue(
                                    "class_id", i)
                                    + "," + bgd.getFieldValue("class_name", i));
                            v_oldSelectedItems.addElement(bgd.getFieldValue(
                                    "class_id", i)
                                    + "," + bgd.getFieldValue("class_name", i));
                        }
                        s_sql = "SELECT uc.class_id AS class_id, t.class_name AS class_name, uc.apply_reason AS apply_reason, uc.approval_id AS approval_id FROM t_user_apply uc, t_class t WHERE uc.user_id = '"
                                + s_userId
                                + "' AND t.class_id = uc.class_id "
                                + s_whereType;
                        bgd.executeSelect(s_sql);
                        //System.out.println(s_sql);
                        for (int i = 0; i < bgd.getRowCount(); i++) {
                            //v_oldSelectedItems.addElement(bgd.getFieldValue("class_id",
                            // i) + "," + bgd.getFieldValue("class_name", i));
                            v_reason.addElement(bgd
                                    .getFieldValue("class_id", i)
                                    + ","
                                    + bgd.getFieldValue("class_name", i).trim()
                                    + ","
                                    + bgd.getFieldValue("apply_reason", i));
                            if (bgd.getFieldValue("approval_id", i).trim()
                                    .equals(""))
                                v_selectedItems.addElement(bgd.getFieldValue(
                                        "class_id", i)
                                        + ","
                                ...
                                v_approve.addElement(bgd.getFieldValue(
                                        "class_id", i));
                        }
                    }

                    //处理课程申请
                    s_countInPage = getParameter(req, "countInPage", true,
                            false, false, "0");
                    i_countInPage = Integer.parseInt(s_countInPage);

                    //取得当前选择的课程，并对原有的数组进行整理
                    for (int i = 0; i < i_countInPage; i++) {
                        s_selectedItem = getParameter(req, "selectItem"
                                + Integer.toString(i), true, false, false, "");
                        if (s_selectedItem.equals(""))
                            continue;
                        if (v_selectedItems.isEmpty()) {
                            v_selectedItems.addElement(s_selectedItem);
                            s_selectedItem = s_selectedItem
                                    + ","
                                    + getParameter(req, "reason"
                                            + Integer.toString(i), true, false,
                                            false, "");
                    ...
                    //去除已被取消的课程
                    s_removeItems = getParameter(req, "removeItems", true,
                            false, false, "");
                    ...
                    //将选择结果传递到JSP
                    session.setAttribute(s_applyType + ".selectedItems",
                            v_selectedItems);
                    session.setAttribute(s_applyType + ".oldSelectedItems",
                            v_oldSelectedItems);
                    session.setAttribute(s_applyType + ".reason", v_reason);
                    session.setAttribute(s_applyType + ".approve", v_approve);
                    String strSubstr = com.htyz.system.SystemProperties
                            .getProperty("db.substr");
                    s_sql = "SELECT * FROM t_class WHERE "
                            + strSubstr
                            + "(class_id, 1, 8) = '"
                            + s_codeId
                            + s_codeValue
                            + "'  AND class_status = '1' "
                            + s_whereType
                            + " AND class_id not in (select class_id from t_user_class uc, t_user u where uc.user_id = u.group_id and u.user_id = '"
                            + s_userId + "') " + " ORDER BY class_id";
                    ...
                    if (s_pageNo.equals("0")) //更新结果
                    {
                        ...
                        for (int i = 0; i < v_oldSelectedItems.size(); i++) {
                            s_classId = (String) v_oldSelectedItems
                                    .elementAt(i);
                            if (v_selectedItems.contains(s_classId)) //将原有的课程激活
                            {
                                s_classId = s_classId.substring(0, 14);
                                s_sql = "UPDATE t_user_class SET status = '1' WHERE user_id = '"
                                        + s_userId
                                        + "' AND class_id = '"
                                        + s_classId + "'";
                                bgd.execute(s_sql);
                            ...
                            {
                                s_classId = s_classId.substring(0, 14);
                                s_sql = "UPDATE t_user_class SET status = '9' WHERE user_id = '"
                                        + s_userId
                                        + "' AND class_id = '"
                                        + s_classId + "'";
                                bgd.execute(s_sql);
                        ...
                        if (s_applyType.equals("free")) {
                            for (int j = 0; j < v_selectedItems.size(); j++) {
                                s_classId = (String) v_selectedItems
                                        .elementAt(j);
                                s_classId = s_classId.substring(0, 14);
                                //删除旧记录
                                s_sql = "DELETE FROM t_user_class WHERE user_id = '"
                                        + s_userId
                                        + "' AND class_id = '"
                                        + s_classId + "'";
                                ...
                                //成功后将session中的数组清除
                                session.removeAttribute(s_applyType
                                        + ".selectedItems");
                                session.removeAttribute(s_applyType
                                        + ".oldSelectedItems");
                                session
                                        .removeAttribute(s_applyType
                                                + ".reason");
                                session.removeAttribute(s_applyType
                                        + ".approve");
                            ...
                            s_sql = "DELETE FROM t_user_apply WHERE user_id = '"
                                    + s_userId
                            ...
                            for (int i = 0; i < v_selectedItems.size(); i++) {
                                ...
                                if (!v_oldSelectedItems.contains(s_classId)) {
                                    //给申请表填记录
                                    for (int j = 0; j < v_reason.size(); j++) {
                                        s_reason = (String) v_reason
                                                .elementAt(j);
                                        if (s_reason.startsWith(s_classId)) {
                                            if (s_reason.length() > s_classId
                                                    .length() + 1)
                                                s_reason = s_reason.substring(
                                                        s_classId.length() + 1,
                                                        s_reason.length());
                                            else
                                                s_reason = "";
                ...
                s_queryId = getParameter(req, "queryid", true, false, false, "null");
                s_queryColoum = getParameter(req, "querycoloum", true, false, false, "null");

                if (!s_pageNo.equals("0") && !s_queryId.equals("null") && !s_queryColoum.equals("null")) {
                    query(req, res);
                    return;
                }

                getServletConfig().getServletContext().getRequestDispatcher(
                        s_display).forward(req, res);
                return;
            }
            ...
            s_page = getParameter(req, "page", false, true, true,
                    "/lesson/lessonApplyTree.jsp");

            getServletConfig().getServletContext().getRequestDispatcher(s_page)
                    .forward(req, res);
            return;
        } catch (Exception ex) {
            try {
    ...
    private void query(HttpServletRequest req, HttpServletResponse res)
        ...
        s_applyType = getParameter(req, "applyType", true, false, false, "free");
        String s_whereType = " AND class_type <> '0' ";
        if (s_applyType.equals("free"))
            s_whereType = " AND class_type = '0' ";
        ...
        s_sql = "SELECT * FROM t_class WHERE "
                ...
                + s_queryId
                + "%' "
                + " AND t_class.class_id not in (select class_id from t_user_class uc, t_user u where uc.user_id = u.group_id and u.user_id = '"
                + s_userId + "') " + s_whereType + " ORDER BY t_class.class_id";
        if (s_queryColoum.equals("class_type")) {
            s_sql = "SELECT t_class.* FROM t_class, t_code WHERE t_class.class_type = t_code.code_value AND "
                    + " t_code.code_id = '0004' AND (t_code.code_namec like '%"
                    + s_queryId
                    + "%' OR t_code.code_value like '%"
                    + s_queryId
                    + "%') "
                    + " AND t_class.class_id not in (select class_id from t_user_class uc, t_user u where uc.user_id = u.group_id and u.user_id = '"
                    + s_userId
        ...
        s_navigation = "<A href=\"" + req.getContextPath()
                + "/\" target=\"_top\">首页</A>"
                + "&gt;<A href=\"lessonApplyManager?applyType=" + s_applyType
                + "\" target=\"_parent\">";
        if (s_applyType.equals("free")) {
            s_navigation = s_navigation + "选择免费课程</A>";
        } else {
        ...
        s_catalog = "搜索";
        s_codeValue = s_codeValue + "&queryid=" + s_queryId + "&querycoloum="
                + s_queryColoum;//增加查询参数
        req.setAttribute("bgd", bgd);
        req.setAttribute("navigation", s_navigation);
        req.setAttribute("catalog", s_catalog);
        req.setAttribute("codeid", s_codeId);
        ...
        req.setAttribute("queryid", s_queryId);
        req.setAttribute("querycoloum", s_queryColoum);
        s_listPage = getParameter(req, "listPage", false, true, true,
                "/lesson/lessonApplyList.jsp");

        getServletConfig().getServletContext().getRequestDispatcher(s_listPage)
                .forward(req, res);
        return;
