...
public class beanChapterList extends HttpServlet implements java.io.Serializable
{
	...
	public void init(ServletConfig conf) throws ServletException
	{
		...
		lessondir = (String) configFile.getString("lessondir");
		workingDir = conf.getServletContext().getRealPath("/") + "/" + lessondir;
	}
			...
			if ( s_action.equalsIgnoreCase("showDetail"))
			{
				...
				else //显示章/节的内容
				{
					...
					if (bcd.getLessonUrl().equals("")&&!fileList.isEmpty())
					{
						...
						s_sql = "UPDATE t_lesson SET lesson_url = '" + res.encodeURL(getPrefixUrl(s_classId, s_lessonId, req.getContextPath()) + filename) + "' WHERE class_id = '" + s_classId + "' AND lesson_id = '" + s_lessonId + "'";
						bgd.execute(s_sql);
						bcd.setLessonUrl(getPrefixUrl(s_classId, s_lessonId, req.getContextPath()) + filename);
					}
					if (!bcd.getLessonUrl().equals("")&&fileList.isEmpty())
					{
					...
					req.setAttribute("prefixUrl", getPrefixUrl(s_classId, s_lessonId, req.getContextPath()));
			...
			if (s_action.equalsIgnoreCase("add")||s_action.equalsIgnoreCase("update"))
			{
				...
				else
				{
					...
					if (s_action.equalsIgnoreCase("add"))
					{
						...
						{
							s_message = "添加";
							s_sql = "INSERT INTO t_lesson VALUES ('"
									+ s_classId + "', '"
									+ s_lessonId + "', '"
									+ s_lessonName + "', '"
									+ s_lessonUrl + "' )";
    ...
    //返回课程的URL前缀
    private String getPrefixUrl(String classid, String lessonid, String contextPath)
    {
    	String s_temp;
//    	s_temp = workingDir.substring(workingDir.indexOf("elearning") - 1, workingDir.length());
//    	if (!s_temp.endsWith("/"))
//    		s_temp = s_temp + "/";
    	s_temp = contextPath + "/" + lessondir + "/" + classid.substring(0, 8) + "/" + classid.substring(8, classid.length()) + "/" + lessonid + "/";
    	return s_temp;
    }
