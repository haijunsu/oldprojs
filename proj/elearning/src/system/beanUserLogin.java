...
public class beanUserLogin extends HttpServlet
{
	...
	{
		//定义变量
		String s_UserId, s_Pass, s_Ver, s_IpAddress,s_Right,s_Group,s_UserName;
		...
		try
		{
			...
			if(bgd.getRowCount() == 0)
			{
				req.setAttribute("errmsg", "口令不正确！");
				getServletConfig().getServletContext().getRequestDispatcher(strErrPage).forward(req, res);
				return;
			}

			if(!(bgd.getFieldValue("user_status",0).equals("1")))
			{
				req.setAttribute("errmsg", "您的帐号还处于申请状态或已被关闭,您无法登录！");
				getServletConfig().getServletContext().getRequestDispatcher(strErrPage).forward(req, res);
				return;
			}
...
///////////////////////////////////////////////////////
			...
			{
				req.setAttribute("errmsg","对不起,您所使用的测试版的测试次数已到,如果您要继续使用,请您与我们联系!电话:010-82031610");
				getServletConfig().getServletContext().getRequestDispatcher(strErrPage).forward(req, res);
				return;
			}
