...
public class SearchBBS extends SecureAction  {
	
	private static com.idn.log.LogWrapper log =
		new com.idn.log.LogWrapper(SearchBBS.class);
	...
	public ActionForward executeme(
		...
		commsearch.util.CommActionLog.setTempLog(
			...
			username,session.getId(),"SearchBBS","B",s_queryid + "0001",0);



		String s_begroup = (String)session.getAttribute("begroup");
		...
		try {
			...
			commsearch.util.CommActionLog.setTempLog(
				Long.parseLong(cdtemp.getNow(cdtemp.FORMAT_ALL_NOSIGN)),
				username,session.getId(),"SearchBBS","E",s_queryid + "0999",l_end - l_begin);


		} catch (Exception e) {
			e.printStackTrace();
			ErrorValue ev = new system.action.ErrorValue();
			ev.setError("SearchBBS","execute","通告一览的主画面出现异常错误:" + e.getMessage());
			...
			commsearch.util.CommActionLog.setTempLog(
				Long.parseLong(cdtemp.getNow(cdtemp.FORMAT_ALL_NOSIGN)),
				username,session.getId(),"SearchBBS","C",s_queryid + "0900",l_end - l_begin);
	...
	String s_queryiddis,String s_ip,String username){
		...
		String[] s_keyall  = null,s_lookin = null;
		...
		try{
			...
			if (s_datashow != null){
				if (s_begintitle.trim().equals("")){
					s_begintitle = "SearchBBS: Now is :" + Integer.toString(s_datashow.length) + "条记录" ;
				} else {
